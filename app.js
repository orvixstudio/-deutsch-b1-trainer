...
function answerText(module, detail){
 var it=state.items[detail.n-1], opts;
 if(module==="Hören"){ opts=it[3]; }
 else { opts=it.q[1]; }
 var found=opts.filter(function(o){return o[1]===detail.ans})[0];
 return found ? found[0] : "Keine Antwort";
}
function correctText(module, detail){
 var it=state.items[detail.n-1], opts;
 if(module==="Hören"){ opts=it[3]; }
 else { opts=it.q[1]; }
 var found=opts.filter(function(o){return o[1]===detail.key})[0];
 return found ? found[0] : "Keine Antwort";
}
function analysis(rec){
 var wrong=rec.details.filter(function(d){return d.ans!==d.key});
 var right=rec.details.filter(function(d){return d.ans===d.key});
 app.innerHTML=top("📊 Ergebnis",rec.module)+'<div class="wrap"><section class="card hero"><div class="resultBig">'+rec.score+" / "+rec.total+'</div><p>'+Math.round(rec.score/rec.total*100)+"% · "+esc(rec.module)+'</p><div class="pillrow"><span class="pill">✓ Richtig: '+right.length+'</span><span class="pill">✕ Falsch: '+wrong.length+'</span></div></section>'+
 '<div class="section">Falsche Antworten ('+wrong.length+')</div>'+
 (wrong.length?wrong.map(function(d){return '<div class="card"><div class="qnum">Aufgabe '+d.n+'</div><div style="margin:6px 0 10px;font-weight:750">'+esc(d.q)+'</div><div class="answerbox"><div class="wrong">✕ Deine Antwort: '+esc(answerText(rec.module,d))+'</div><div class="correct" style="margin-top:7px">✓ Richtige Antwort: '+esc(correctText(rec.module,d))+'</div></div></div>'}).join(""):'<div class="card"><span class="correct">✓ Alle Antworten richtig!</span></div>')+
 '<div class="section">Richtige Antworten ('+right.length+')</div>'+
 (right.length?right.map(function(d){return '<div class="card"><div class="qnum">Aufgabe '+d.n+'</div><div style="margin:6px 0 8px;font-weight:750">'+esc(d.q)+'</div><div class="answerbox"><div class="correct">✓ Deine Antwort: '+esc(answerText(rec.module,d))+'</div></div></div>'}).join(""):"")+
 '<div class="grid"><button class="btn" id="again">🔄 Noch einmal</button><button class="btn secondary" id="rh">📊 Verlauf</button></div></div>';
